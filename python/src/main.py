...
import pika
paramaters = (pika.ConnectionParameters(host=os.environ['BROKER_HOST'], port=os.environ['BROKER_PORT']))
...
def setup_model(args):
	...
	ds.enableDecoderWithLM(args['alphabet'], args['binary'], args['trie'], LM_ALPHA, LM_BETA)
	return ds
...
def make_handler_interpret_audio(ds):
	def handler(audio):
		...
		if audio_length % 16 == 0:
			...
			connection.basic_publish(exchange='audio', routing_key='interpreted', reply)
		else:
...
def main():
	...
	for method_frame, properties, body in channel.consume('audio'):
	    # Display the message parts and acknowledge the message
	    print(method_frame, properties, len(body))
